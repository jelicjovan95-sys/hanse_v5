...
with pd.ExcelWriter('CRM_Field_Mapping_Analysis.xlsx', engine='openpyxl') as writer:
    pd.DataFrame(sheet1[1:], columns=sheet1[0]).to_excel(writer, sheet_name='Master Field Mapping', index=False)
    pd.DataFrame(sheet2[1:], columns=sheet2[0]).to_excel(writer, sheet_name='Document Contribution', index=False)
    pd.DataFrame(sheet3[1:], columns=sheet3[0]).to_excel(writer, sheet_name='Lead Card Inventory', index=False)
    pd.DataFrame(sheet4[1:], columns=sheet4[0]).to_excel(writer, sheet_name='Master Safety Inventory', index=False)
    pd.DataFrame(sheet5[1:], columns=sheet5[0]).to_excel(writer, sheet_name='Safety Score Inputs', index=False)
    pd.DataFrame(sheet6[1:], columns=sheet6[0]).to_excel(writer, sheet_name='Compliance Inputs', index=False)
    pd.DataFrame(sheet7[1:], columns=sheet7[0]).to_excel(writer, sheet_name='Missing Data Report', index=False)
    pd.DataFrame(sheet8[1:], columns=sheet8[0]).to_excel(writer, sheet_name='Validation Report', index=False)
